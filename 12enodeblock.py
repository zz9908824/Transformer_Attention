...
class EncodeBlock(nn.Module):
    def __init__(self, d_model, n_heads, hidden_dim, dropout=0.1):
        super().__init__()
        self.mha = MHA_Res(d_model, n_heads, dropout)
...
class Encoder(nn.Module):
    def __init__(self, d_model, n_heads, hidden_dim, dropout=0.1) :
        ...
        self.encode_blocks = nn.ModuleList([
                EncodeBlock(d_model, n_heads, hidden_dim, dropout) for _ in range(6)
            ])
    def forward(self,x):
...
encoder = Encoder(d_model, head_nums, hidden_dim, dropout)
